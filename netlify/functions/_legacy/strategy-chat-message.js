...
exports.handler = async (event) => {
  ...
  if (!runId || (action !== "clear" && (!text || text.length > 12000))) return { statusCode: 400, headers: cors(), body: JSON.stringify({ error: "runId and a message under 12,000 characters are required." }) };
  try {
    ...
    if (action === "clear") {
      await fbSet(`strategy_runs/${runId}/chatMessages`, null);
