...
3. Preserve at least eight real audience questions in their original wording. Each must cite a source ID. Do not paraphrase a marketer's imagined question into quotation-like language. In the human-facing handoff, treat these as "Real audience questions": actual questions and doubts people are already asking, plus what each one reveals.
...
## Team-readable handoff

- The output must be understandable to account managers, designers, copywriters and editors without knowing the internal agent schema.
- Avoid internal labels in prose such as "live questions". Use plain explanations like "real audience questions", "what people are unsure about", "what this tells us" and "why it matters for strategy".
- For each question, make the underlying need useful: explain the concern, desire, hesitation or context it reveals.

## Quality test

Before returning, remove any audience question that sounds like agency language. Remove any insight the strategy agent could have guessed without research. The finished output should contain friction, not a polished category overview.
`;
...
const CONCEPT_REFINE_PROMPT = `# Agent 6 — Concept refinement
...
const COPY_REFINE_PROMPT = `# Agent 7 — Copy refinement
