// BB's 30 IST message to the team. The work lives in lib/strategy/digest-run.js — this file
...
//
// Also reachable by hand (GET /.netlify/functions/bb-digest-evening) for testing; the
...
export default async function () {
  try {
    const result = await digestRun.runDigest("evening");
    return new Response(JSON.stringify({ success: true, ...result }), { headers: { "Content-Type": "application/json" } });
  } catch (error) {
    console.error("bb-digest-evening failed:", error);
