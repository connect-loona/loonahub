...
//
// Also reachable by hand (GET /.netlify/functions/bb-digest-midday) for testing; the
...
export default async function () {
  try {
    const result = await digestRun.runDigest("midday");
    return new Response(JSON.stringify({ success: true, ...result }), { headers: { "Content-Type": "application/json" } });
  } catch (error) {
    console.error("bb-digest-midday failed:", error);
