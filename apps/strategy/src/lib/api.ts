...
export function askBB(args: { brandId?: string; scope?: "global"; threadId?: string; message: string; actor: string; clientMessageId: string; attachments?: Array<{ assetKey: string; url: string; filename?: string }> }): Promise<{ ok: true; pending: true; messageId: string }> {
  return post("strategy-bb-chat", args) as Promise<{ ok: true; pending: true; messageId: string }>;
}
...
export async function saveBrand(args: { brandId: string; config: Record<string, unknown> }): Promise<{ ok: true }> {
  ...
  if (!res.ok) {
    ...
    // A configuration is usable only once the authoritative endpoint has validated and
    // stored it. Never fall back to a raw client write, which can create an unreadable
    // record that looks configured in the UI but fails the planning runtime.
    throw new Error(msg);
  }
