...
export function BrandForm({ brandId, initialBrand, onCancel, onSaved }: {
  ...
  const [reel, setReel] = useState(String(b?.deliverables?.reel ?? 6));
  const [carousel, setCarousel] = useState(String(b?.deliverables?.carousel ?? 4));
  const [staticCount, setStaticCount] = useState(String(b?.deliverables?.static ?? 3));
  ...
  async function handleSubmit() {
    ...
    const config = {
      ...
      deliverables: {
        reel: parseInt(reel, 10) || 0,
        carousel: parseInt(carousel, 10) || 0,
        static: parseInt(staticCount, 10) || 0,
        confirmed,
      },
      voice: {
    ...
    <div>
      ...
      <div className="st-board">
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
          <div style={{ flex: 1 }}><label className="st-field-label">Reels</label><input className="st-form-control" type="number" min={0} value={reel} onChange={(e) => setReel(e.target.value)} /></div>
          <div style={{ flex: 1 }}><label className="st-field-label">Carousels</label><input className="st-form-control" type="number" min={0} value={carousel} onChange={(e) => setCarousel(e.target.value)} /></div>
          <div style={{ flex: 1 }}><label className="st-field-label">Statics</label><input className="st-form-control" type="number" min={0} value={staticCount} onChange={(e) => setStaticCount(e.target.value)} /></div>
        </div>
